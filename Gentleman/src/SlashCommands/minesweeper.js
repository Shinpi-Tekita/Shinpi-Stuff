...
interaction.editReply({ embeds:[game.setDescription(`**${interaction.user}'s Minesweeper Game**\nBlocks Found: ${block}`)], components: [row1, row2, row3, row4, row5] })
})
...
interaction.editReply({ embeds: [game.setDescription(`**${interaction.user}'s Minesweeper Game**\nBlocks Found: ${block}`)], content: `🏆 YOU WON 🎉`,components: [row1, row2, row3, row4, row5] }) 
}else{
positions.forEach((g) => {
 g.a = g.r
row1 = new Discord.MessageActionRow()
...
interaction.editReply({ embeds: [game.setDescription(`**${interaction.user}'s Minesweeper Game**\nBlocks Found: ${block}`)], content: `💥 *Explosion Sounds*`,components: [row1, row2, row3, row4, row5] }) 
}
});
},
};
